                  ...
                  <div key={t.name} className="p-3 rounded-lg border border-gray-200 dark:border-gray-700 text-sm">
                    ...
                    {t.messages.length > 0 && (
                      <div className="mt-2 space-y-1">
                        {t.messages.map((m, i) => <code key={i} className="block text-[10px] px-2 py-1 rounded bg-gray-100 dark:bg-gray-800 text-bleepx-text font-mono truncate" title={m}>{m}</code>)}
                      </div>
                    )}
                  </div>
                ))}
