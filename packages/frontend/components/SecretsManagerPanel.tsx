              ...
              <div key={s.name} className="p-3 rounded-lg border border-gray-200 dark:border-gray-700 text-sm">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    <button onClick={() => onAction(rotateSecret(state, s.name))} className="text-xs px-2.5 py-1.5 rounded-full bg-sky-100 dark:bg-sky-900/30 text-sky-700 dark:text-sky-300 font-bold hover:bg-sky-200 transition-colors">Rotate</button>
                    <button onClick={() => onAction(deleteSecret(state, s.name, true))} className="text-xs px-2.5 py-1.5 rounded-full bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-300 font-bold hover:bg-rose-200 transition-colors">Delete</button>
                  </div>
                </div>
                ...
                <div className="mt-2 flex items-center gap-2 min-w-0">
                  ...
                  <button onClick={() => setReveal((prev) => ({ ...prev, [s.name]: !prev[s.name] }))} className="text-xs px-2 py-1 rounded-full bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 font-bold hover:bg-gray-300 transition-colors flex-shrink-0">
                    {reveal[s.name] ? 'Hide' : 'Reveal'}
                  </button>
                </div>
                {s.versionStages.includes('AWSPENDING') && (
                  <div className="text-xs text-amber-700 dark:text-amber-400 mt-1">Pending rotation version exists</div>
                )}
              </div>
            ))}
          </div>
