                ...
                {volumes.map((v) => (
                  <div key={v.volumeId} className="p-3 rounded-lg border border-gray-200 dark:border-gray-700 text-sm">
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      ...
                      <span className="text-xs px-2 py-0.5 rounded-full bg-sky-100 text-sky-700 flex-shrink-0">{v.volumeType}</span>
                    </div>
                    ...
                    <div className="flex gap-2 mt-2">
                      {!v.attachedTo && attachTarget && <button onClick={() => onAction(attachEBSVolume(state, v.volumeId, attachTarget))} className="text-xs px-2 py-1 rounded bg-emerald-100 text-emerald-700 hover:bg-emerald-200 font-bold">Attach</button>}
                      {!v.attachedTo && <button onClick={() => onAction(deleteEBSVolume(state, v.volumeId))} className="text-xs px-2 py-1 rounded bg-rose-100 text-rose-700 hover:bg-rose-200 font-bold">Delete</button>}
                    </div>
                  </div>
                ))}
              </div>
                ...
                {fss.map((fs) => (
                  <div key={fs.creationToken} className="p-3 rounded-lg border border-gray-200 dark:border-gray-700 text-sm">
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      ...
                      <span className="text-xs px-2 py-0.5 rounded-full bg-violet-100 text-violet-700 flex-shrink-0">{fs.performanceMode}</span>
                    </div>
                    ...
                    <div className="flex gap-2 mt-2">
                      ...
                      <button onClick={() => efsPolicy && onAction(addEFSLifecyclePolicy(state, fs.creationToken, efsPolicy))} className="text-xs px-2 py-1 rounded bg-emerald-100 text-emerald-700 hover:bg-emerald-200 font-bold">Add</button>
                      <button onClick={() => onAction(deleteEFSFileSystem(state, fs.creationToken))} className="text-xs px-2 py-1 rounded bg-rose-100 text-rose-700 hover:bg-rose-200 font-bold">Delete</button>
                    </div>
                  </div>
                ))}
              </div>
