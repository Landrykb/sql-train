            ...
            {machines.map((m) => (
              <div key={m.name} className="p-3 rounded-lg border border-gray-200 dark:border-gray-700 text-sm">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <strong className="text-bleepx-text">{m.name}</strong>
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-violet-100 dark:bg-violet-900/20 text-violet-700 dark:text-violet-300 font-bold">{m.type}</span>
                </div>
                <pre className="mt-2 p-2 rounded-lg bg-gray-900 text-green-400 text-[10px] font-mono overflow-x-auto">{m.definition}</pre>
