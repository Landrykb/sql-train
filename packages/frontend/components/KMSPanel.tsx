            ...
            {Object.values(state.kms.keys).map((key) => (
              <div key={key.keyId} className="p-3 rounded-lg border border-gray-200 dark:border-gray-700 text-sm">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <strong className="text-bleepx-text min-w-0 break-words">{key.alias || key.keyId}</strong>
                  <span className={`text-xs px-2 py-0.5 rounded-full font-bold flex-shrink-0 ${key.enabled ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>{key.enabled ? 'Enabled' : 'Disabled'}</span>
                </div>
                <div className="text-xs text-bleepx-text-secondary mt-1 break-words">{key.keySpec} · {key.keyUsage} · {key.description}</div>
                <button onClick={() => onAction(setKMSKeyEnabled(state, key.keyId, !key.enabled))} className="mt-2 text-xs px-2.5 py-1.5 rounded-full bg-sky-100 dark:bg-sky-900/30 text-sky-700 dark:text-sky-300 font-bold hover:bg-sky-200 transition-colors">{key.enabled ? 'Disable' : 'Enable'}</button>
              </div>
            ))}
          </div>
        </div>
