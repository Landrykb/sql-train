...
export default async function LabProjectPage({ params }: { params: Promise<{ domain: string; projectId: string }> }) {
  ...
  return (
    <main className="max-w-4xl mx-auto px-2 md:px-4 lg:px-6 py-4 space-y-4 bg-bleepx-bg min-h-screen pb-20 min-w-0">
      {/* Breadcrumb */}
      ...
      <div className="flex flex-wrap items-start justify-between">
        <div className="min-w-0">
          <h1 className="text-2xl sm:text-3xl font-bold text-bleepx-text break-words">{doc.name}</h1>
          <p className="text-sm text-bleepx-text-secondary mt-1 break-words">{doc.description}</p>
        </div>
        <CaseInterpretationButton 
          verse="lab" 
          itemId={projectId} 
          itemName={`${doc.name} - ${meta.name}`} 
          domain={domain} 
        />
      </div>
