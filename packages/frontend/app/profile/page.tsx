...
export default function ProfilePage() {
  ...
  const handleSignIn = (provider: string) => {
    playBleep();
    // For now, simulate sign-in with localStorage
    if (provider === 'github') {
      const username = prompt('*bleep* Enter your GitHub username, human:');
      if (username) {
        saveProfile({ authProvider: 'github', githubUsername: username, displayName: username });
      }
    } else if (provider === 'google') {
      const email = prompt('*bleep* Enter your email, human:');
      if (email) {
        saveProfile({ authProvider: 'google', email, displayName: email.split('@')[0] });
      }
    } else if (provider === 'email') {
      const email = prompt('*bleep* Enter your email address:');
      if (email) {
        saveProfile({ authProvider: 'email', email, displayName: email.split('@')[0] });
      }
    }
    setShowSignIn(false);
  };
  ...
  const handleConnectGitHub = () => {
    playBleep();
    const username = prompt('*bleep* Enter your GitHub username to connect:');
    if (username) {
      saveProfile({ githubUsername: username });
